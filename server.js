...
async function initDB() {
  try {
    ...
    const ordersColumnsToCheck = [
      { name: 'code_requested', type: 'BOOLEAN DEFAULT FALSE' },
      { name: 'wrong_code_attempts', type: 'INTEGER DEFAULT 0' }
    ];
    
    for (const column of ordersColumnsToCheck) {
      try {
        await pool.query(`
          ALTER TABLE orders 
          ADD COLUMN IF NOT EXISTS ${column.name} ${column.type}
        `);
        console.log(`ℹ️ Столбец ${column.name} проверен в таблице orders`);
      } catch (e) {
        console.log(`ℹ️ Столбец ${column.name} уже существует:`, e.message);
      }
    }
...
// ===== ДОПОЛНИТЕЛЬНЫЕ API ДЛЯ ФРОНТЕНДА =====
app.get('/api/user/orders', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT order_id as id, total, status, created_at as date, items
       FROM orders 
       WHERE user_id = $1 
       ORDER BY created_at DESC`,
      [userId]
...
app.post('/api/user/orders/update-status', async (req, res) => {
  try {
